	...
	function appendFilters(oTable, args, table_selector) {
		...
		for (columnObjKey in args) {
			if (args.hasOwnProperty(columnObjKey)) {
				...
				//OPTGROUP
				var optgroup_list = [];
				var column_data_splited = [];
				var reccord = {};
				var optgroup = {};

				for (ii = 0; ii < column_data.length; ii++) {
					if(typeof column_data[ii] !== 'object' ) {
						reccord = {value: column_data[ii], label: column_data[ii]};
					} else {
				   		reccord = column_data[ii];
					}
					
					if (reccord.optgroup === undefined) {
						optgroup = "_DEFAULT";
					} else {
						optgroup = reccord.optgroup;
					}
	
	  				if (optgroup_list.indexOf(optgroup) < 0 ) {
	     					optgroup_list.push(optgroup);
	     					column_data_splited[optgroup_list.indexOf(optgroup)] = [];
	  				}
				...
				if ($filter_selector.length === 1) {
					...
					if (columnObj.filter_type === "select" || columnObj.filter_type === 'custom_func') {
						...
						$(filter_selector_string).append(exclude_str + "<input type=\"text\" id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter\" onmousedown=\"yadcf.stopPropagation(event);\" onclick='yadcf.stopPropagation(event);"
							+ "' placeholder='" + filter_default_label + "'" + " filter_match_mode='" + filter_match_mode + "' " + filterActionStr + "></input>");
